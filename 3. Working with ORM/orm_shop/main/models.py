...
class Car(models.Model):
    ...
    drive_unit = models.CharField('Привод', max_length=20, choices=DRIVE_UNIT_CHOICES)
    gearbox = models.CharField('Коробка передач', max_length=20, choices=GEARBOX_CHOICES)
    fuel_type = models.CharField('Тип топлива', max_length=20, choices=FUEL_TYPE_CHOICES)
    price = models.DecimalField('Цена', max_digits=10, decimal_places=2)
    image = models.ImageField('Изображение авто', upload_to='images/')
...
class Sale(models.Model):
    ...
    def __str__(self):
        return f'Дата, время {self.created_at} клиенту {self.client} продан автомобиль {self.car}'